...
- **責務 (Responsibility):** アプリケーションのメインウィンドウとして機能し、以下の責務を負う。
    ...
    - **状態の永続化:** アプリケーション終了時にウィンドウのジオメトリとドックレイアウトの状態を `QSettings` に保存し、次回起動時に復元する。
    - ユーザーアクション（メニュー、ツールバー）のハンドリング。
    - デバッガの実行制御（Run, Stop, Step）と、スナップショットに基づく全UIコンポーネントの更新調整。
    - **ファイルのロード:** Intel HEXファイルに加え、アセンブリソースコード（.asm）のロード機能を備える。アセンブリロード時は抽出されたシンボル情報をバックエンドと共有する。
...
- **重要なアルゴリズム (Key Algorithms):**
    ...
    - **プログラムロード連携:** アセンブリロード時に `AssemblyLoader` から返された `SymbolMap` を `cpu.set_symbol_map()` および `code_view.set_symbol_map()` に即座に反映させる。
...
#### 4.8. CodeView (コードビュー)
- **責務 (Responsibility):** メモリ上の機械語を逆アセンブルして表示し、現在のプログラムカウンタ (PC) の位置を視覚的に強調する。また、読み込まれたシンボル（ラベル）情報をアドレスに対応させて表示する。
- **提供するAPI (Public API):**
    - `update_code(bus: Bus, pc: int) -> None`:
        - 指定されたPC周辺のコードを逆アセンブルし、テーブルを更新する。PCに該当する行をハイライト表示する。
    - `set_symbol_map(self, symbol_map: Dict[str, int]) -> None`:
        - **責務:** デバッグシンボル情報（アドレスとラベルの対応）を設定する。逆アセンブル表示時に、アドレスに対応するラベルが存在すればそれを併記するために使用される。
